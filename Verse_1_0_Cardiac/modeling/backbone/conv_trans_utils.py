...
class RelativePositionBias(nn.Module):
    ...
    def forward(self, H, W):
        # relative_position_bias_table = self.relative_position_bias_table.to(self.device)
...
class up_block_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, heads=4, dim_head=64, attn_drop=0., proj_drop=0.,
                 reduce_size=16, projection='interp', rel_pos=True, stride=1):
        super().__init__()
        ...
        block_list.append(block(2 * out_ch, out_ch, stride=stride))
